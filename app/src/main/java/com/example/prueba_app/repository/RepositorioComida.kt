...
import com.example.prueba_app.model.ItemCarrito
...
// AHORA implementa la interfaz ComidaDataSource
class RepositorioComida(private val db: AppDatabase) : ComidaDataSource {
    ...
    override suspend fun inicializarDatos() {
        ...
        if (db.platoDao().contarPlatos() == 0) {
            val platosIniciales = listOf(
                ...
                Plato(
                    nombre = "Lomo Saltado",
                    descripcion = "Trozos de carne salteados.",
                    precio = 14500,
                    imagenUrl = "lomo"
                ),
                Plato(
                    nombre = "Ají de Gallina",
                    descripcion = "Pollo deshilachado en crema de ají.",
                    precio = 10500,
                    imagenUrl = "aji"
                ),
                Plato(
                    ...
                    precio = 9000,
                    imagenUrl = "anticuchos"
                ),
                Plato(
                    nombre = "Causa Limeña",
                    descripcion = "Masa de papa con pollo.",
                    precio = 9500,
                    imagenUrl = "causa"
                ),
                Plato(
                    nombre = "Rocoto Relleno",
                    descripcion = "Rocoto horneado relleno de carne.",
                    precio = 11000,
                    imagenUrl = "rocoto"
                ),
                Plato(
                ...
                Plato(
                    ...
                    descripcion = "Arroz y frijoles con lomo.",
                    precio = 13500,
                    imagenUrl = "tacutacu"
                )
            )
        ...
        try {
            if (usuario.id == 0) {
                ...
                if (response.isSuccessful && response.body() != null) {
                    ...
                    Log.e(
                        "API",
                        "Error Signup: ${response.code()} - ${response.errorBody()?.string()}"
                    )
                }
            } else {
                ...
                if (response.isSuccessful) {
                    ...
                    Log.e(
                        "API",
                        "Error Update: ${response.code()} - ${response.errorBody()?.string()}"
                    )
                }
            }
        } catch (e: Exception) {
    ...
    override val carrito: Flow<List<DetalleCarrito>> =
        db.carritoDao().obtenerCarritoConDetalles()

    override suspend fun agregarAlCarrito(platoId: Int, userId: Int) {
        try {
            if (userId != -1) {
                ...
                if (!response.isSuccessful) {
                    Log.e("API", "Error AddCart: ${response.code()}")
                }
            }
        } catch (e: Exception) {
            Log.e("API", "Ex AddCart: ${e.message}")
        }

        val itemExistente = db.carritoDao().obtenerItemPorPlato(platoId)
        if (itemExistente != null) {
            db.carritoDao().actualizarItem(
                itemExistente.copy(cantidad = itemExistente.cantidad + 1)
            )
        } else {
            db.carritoDao().insertarItem(
                ItemCarrito(platoId = platoId, cantidad = 1)
            )
        }
    }

    override suspend fun reducirCantidadOEliminar(detalle: DetalleCarrito, userId: Int) {
        try {
            if (userId != -1) {
                ...
                if (!response.isSuccessful) {
                    Log.e("API", "Error RemoveCart: ${response.code()}")
                }
            }
        } catch (e: Exception) {
            Log.e("API", "Ex RemoveCart: ${e.message}")
        }
    ...
    override suspend fun eliminarItemCarritoPorId(idItem: Int) {
